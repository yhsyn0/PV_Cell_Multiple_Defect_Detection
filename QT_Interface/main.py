...
import inspect, threading, subprocess, os, socket, sys, time
from PyQt5 import QtCore, QtGui, QtWidgets
from VideoThread import VideoThread
from ioFuncs import takeImg, sendCmd
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.irProcess.setEnabled(False)
        ...
        self.rgbProcess = QtWidgets.QPushButton(self.centralwidget)
        self.rgbProcess.setEnabled(False)
        self.rgbProcess.setHidden(True)
        self.rgbProcess.setGeometry(QtCore.QRect(600, 773, 100, 25))
        self.rgbProcess.setObjectName("rgbProcess")
        #self.rgbProcess.clicked.connect(self.rgbProcessAction)

        self.thermalProcess = QtWidgets.QPushButton(self.centralwidget)
        self.thermalProcess.setEnabled(False)
        self.thermalProcess.setGeometry(QtCore.QRect(600, 437, 100, 25))
        self.thermalProcess.setObjectName("thermalProcess")
        self.thermalProcess.clicked.connect(self.thermalProcessAction)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.thermalProcess.setText(_translate("MainWindow", "Process"))
    ...
    def irConnectAction(self):
        ...
        self.threadIR = VideoThread("irVideo", self.irVideo, self.irIP, self.irConnect, self.irDisconnect, self.irProcess, self.logText, HOST, PORT)
    ...
    def irDisconnectAction(self):
        ...
        if inspect.stack()[1][3] == "irProcessAction":
            self.irConnect.setEnabled(False)
            self.irDisconnect.setEnabled(False)
            self.irProcess.setEnabled(False)
            self.irIP.setEnabled(False)
    ...
    def thermalConnectAction(self):
        ...
        self.threadThermal = VideoThread("thermalVideo", self.thermalVideo, self.thermalIP, self.thermalConnect, self.thermalDisconnect, self.thermalProcess, self.logText, HOST, PORT)
    ...
    def rgbConnectAction(self):
        ...
        self.threadRGB = VideoThread("rgbVideo", self.rgbVideo, self.rgbIP, self.rgbConnect, self.rgbDisconnect, self.rgbProcess, self.logText, HOST, PORT)
    ...
    def irProcessAction(self):
        ...
        sendCmd(HOST, PORT, "irServerKill")
        """time.sleep(3)
        sendCmd(HOST, PORT, "irTakePhoto")"""
        """
        time.sleep(5)
        ...
        print("Started")
        if subprocess.run(["python3", "/home/azarakuss/development/xavier/transfer.py", 
            "/home/azarakuss/development/sources/captures/" + 
            sorted(os.listdir("/home/azarakuss/development/sources/captures/"), reverse=True)[0]],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0:
        
            print("ERRORR")"""
        

        #self.irConnectAction()

    def thermalProcessAction(self):
        nowStr = datetime.now().strftime("%d-%m-%   Y_%H-%M-%S")
        name = "/home/azarakuss/development/xavier/thermal/" + nowStr + ".jpg"
        cv2.imwrite("/home/azarakuss/development/xavier/records/thermal/" + nowStr + ".jpg", self.threadThermal.frame)
        print(type(self.threadThermal.frame))
        print("saved")
